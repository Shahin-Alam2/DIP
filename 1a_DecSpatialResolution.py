...
def make_gray_image(image):
    gray_image = np.mean(image, axis=2).astype(np.uint8) #axis=2 mane channel,0 hole row,1 hole col
    return gray_image                                    #channel er mean ba avg ar astype eta data type default unsigned intgr
...
rgb_image = plt.imread(r'D:\Odd\Image Processing\Lab\myself\images\butterfly.jpeg')
...
row, col = 2, 4
fig, ax = plt.subplots(row, col, figsize=(9, 7)) #fig size holo display window er width height
